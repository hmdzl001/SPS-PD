...
public class LootIndicator extends Tag {
	...
	protected void createChildren() {
		...
		slot.showParams(true, false, false, false);
		add(slot);
	}
	...
	public void update() {
		...
		if (Dungeon.hero.ready) {
			Heap heap = Dungeon.depth.heaps.get(Dungeon.hero.pos);
			if (heap != null) {

				Item item = 
				    heap.type == Heap.Type.CHEST || heap.type == Heap.Type.MIMIC ? ItemSlot.CHEST :
				    heap.type == Heap.Type.LOCKED_CHEST || heap.type == Heap.Type.G_MIMIC ? ItemSlot.LOCKED_CHEST: 
