...
public class TriangleWLevel extends RegularLevel {
	...
	public static void spawn(TriangleWLevel level, Room room) {
		int pos;
		do {pos = room.random();}
	...
	protected boolean assignRoomType() {
		...
		wisdomspecial = new ArrayList<Room.Type>(Arrays.asList(Type.WISDOM_ROOM));

			for (Room r : rooms) {
				if (r.type == Type.NULL && r.connected.size() == 1) {

					if (wisdomspecial.size() > 0 && r.width() > 3 && r.height() > 3) {

						
						int n = wisdomspecial.size();
						r.type = wisdomspecial.get(Math.min(Random.Int(n),Random.Int(n)));
