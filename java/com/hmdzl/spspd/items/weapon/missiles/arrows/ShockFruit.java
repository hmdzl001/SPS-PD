...
public class ShockFruit extends Arrows {
	...
	protected void onThrow(int cell) {
		...
		if (enemy == null || enemy == curUser) {
			GameScene.add(Blob.seed(cell, 4, ElectriShock.class));
			for (int n : Level.NEIGHBOURS8) {
				int c = cell + n;
				if (c >= 0 && c < Level.getLength()) {
					GameScene.add(Blob.seed(c, 4, ShockEffectDamage.class));
				}
			}
		} else
			super.onThrow(cell);
