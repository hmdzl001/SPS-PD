...
public class WandOfFireElement extends DamageWand {
                   
	{
		image = ItemSpriteSheet.WANDOFE_FIRE;
	...
	protected void onZap(Ballistica bolt) {
		...
		int level = level();
		Buff.prolong( curUser, FireBless.class, chargesPerCast());
		...
		for (int n : Floor.NEIGHBOURS9) {
			...
			if (c >= 0 && c < Floor.getLength()) {
				 ...
				 GameScene.add( Blob.seed(c,chargesPerCast()+5, FireEffectDamage.class ) );
