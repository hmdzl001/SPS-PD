...
import com.hmdzl.spspd.actors.buffs.blessbuff.IceBless;
...
public class WandOfIceElement extends DamageWand {
                   
	{
		image = ItemSpriteSheet.WANDOFE_ICE;
	...
	protected void onZap(Ballistica bolt) {
		...
		int level = level();
		Buff.prolong( curUser, IceBless.class, chargesPerCast());
		...
		for (int n : Floor.NEIGHBOURS9) {
			...
			if (c >= 0 && c < Floor.getLength()) {
				GameScene.add( Blob.seed(c,chargesPerCast()+5, IceEffectDamage.class ) );
	...
	protected void fx(Ballistica bolt, Callback callback) {
		MagicMissile.coldLight(curUser.sprite.parent, curUser.pos, bolt.collisionPos, callback);
