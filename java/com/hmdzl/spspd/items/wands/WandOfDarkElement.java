...
import com.hmdzl.spspd.actors.buffs.blessbuff.DarkBless;
...
public class WandOfDarkElement extends DamageWand {
                   
	{
		image = ItemSpriteSheet.WANDOFE_DARK;
	...
	protected void onZap(Ballistica bolt) {
		...
		int level = level();
		Buff.prolong( curUser, DarkBless.class, chargesPerCast());
		...
		for (int n : Floor.NEIGHBOURS9) {
			...
			if (c >= 0 && c < Floor.getLength()) {
				...
				GameScene.add( Blob.seed(c,chargesPerCast()+5, DarkEffectDamage.class ) );
	...
	protected void fx(Ballistica bolt, Callback callback) {
		MagicMissile.shadow(curUser.sprite.parent, curUser.pos, bolt.collisionPos, callback);
