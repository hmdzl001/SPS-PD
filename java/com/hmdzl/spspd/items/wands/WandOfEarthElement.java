...
import com.hmdzl.spspd.actors.buffs.blessbuff.EarthBless;
...
public class WandOfEarthElement extends DamageWand {
                   
	{
		image = ItemSpriteSheet.WANDOFE_EARTH;
	...
	protected void onZap(Ballistica bolt) {
		...
		int level = level();
		Buff.prolong( curUser, EarthBless.class, chargesPerCast());
		...
		for (int n : Floor.NEIGHBOURS9) {
			...
			if (c >= 0 && c < Floor.getLength()) {
				 ...
				 GameScene.add( Blob.seed(c,chargesPerCast()+5, EarthEffectDamage.class ) );
