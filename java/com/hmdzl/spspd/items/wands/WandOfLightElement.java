...
import com.hmdzl.spspd.actors.buffs.blessbuff.LightBless;
...
public class WandOfLightElement extends DamageWand {
                   
	{
		image = ItemSpriteSheet.WANDOFE_LIGHT;
	...
	protected void onZap(Ballistica bolt) {
		...
		Buff.prolong( curUser, LightBless.class, chargesPerCast());
		...
		for (int n : Floor.NEIGHBOURS9) {
			...
			if (c >= 0 && c < Floor.getLength()) {
				 GameScene.add( Blob.seed(c,chargesPerCast()+5, LightEffectDamage.class ) );
	...
	protected void fx(Ballistica bolt, Callback callback) {
		MagicMissile.whiteLight(curUser.sprite.parent, curUser.pos, bolt.collisionPos, callback);
