...
import com.hmdzl.spspd.ui.RenderedTextBlock;
...
public class FloatingText2 extends RenderedTextBlock {
	...
	public static int ROBOT = 12;
	public static int FALLING    = 13;
	...
	private static final SparseArray<ArrayList<FloatingText2>> stacks = new SparseArray<>();
	
	public FloatingText2() {
		super(7*PixelScene.defaultZoom);
	...
	public static void show( float x, float y, int key, String text, int color, int iconIdx,boolean left ) {
			...
			FloatingText2 txt = GameScene.status2();
	...
	private static void push( FloatingText2 txt, int key ) {

		synchronized (stacks) {
			txt.key = key;

			ArrayList<FloatingText2> stack = stacks.get(key);
			if (stack == null) {
				stack = new ArrayList<>();
			...
			if (stack.size() > 0) {
				FloatingText2 below = txt;
				int aboveIndex = stack.size() - 1;
				int numBelow = 0;
				while (aboveIndex >= 0) {
					numBelow++;
					FloatingText2 above = stack.get(aboveIndex);
					if (above.bottom() + 4 > below.top()) {
						above.setPos(above.left(), below.top() - above.height()+2);
