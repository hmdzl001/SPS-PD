		...
		public void onSelect(Item item) {
			if (item != null && item instanceof Scroll && item.isIdentified()) {
				...
				if (consumedpts > levelChk && level()<10) {
					upgrade();
					GLog.p(Messages.get(EyeOfSkadi.class, "infuse_scroll"));
					}
				
			
			} else if (item instanceof Scroll && !item.isIdentified()){
				GLog.w(Messages.get(EyeOfSkadi.class, "unknown_scroll"));
		   } else if (item != null){
			GLog.w(Messages.get(EyeOfSkadi.class, "unable_scroll"));
		}
	 }
	};
