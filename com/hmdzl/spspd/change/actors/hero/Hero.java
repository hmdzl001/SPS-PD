...
public class Hero extends Char {
	...
	@Override
	public int dr() {
		int dr = belongings.armor != null ? Math.max(belongings.armor.DR, belongings.armor.level)
				: 0;
	...
	public int damageRoll() {
		...
		if (bonus > 0){ dmg *= Math.min(2f,(1f + (bonus/30)*1f));}
	...
	public float speed() {
		...
		if (aEnc > 0) {
			return (float) (speed * Math.pow(1.75, -aEnc));
		} else {

			return ((HeroSprite) sprite)
					.sprint(subClass == HeroSubClass.FREERUNNER
							&& !isStarving()) ? invisible > 0 ? 2.5f * speed
					: 1.5f * speed : speed;
	...
	public boolean act() {
		...
		if ((Statistics.time > 360 && Statistics.time <601 ) || (Statistics.time > 840 && Statistics.time < 1081 )) {
			viewDistance = 6;
			Dungeon.observe();
				...
				if (item.doPickUp(this)) {
					...
					if (item instanceof Dewdrop
						...
						if (item instanceof DewVial) {
							GLog.w(Messages.get(this, "dewvial"));
