		...
		if (Dungeon.dewDraw || Dungeon.dewWater){
		  ...
		  for (int n : Level.NEIGHBOURS9) {
			 ...
			 if (c >= 0 && c < Level.getLength() && Level.passable[c]) {
				...
				if (Random.Int(12)==1){Dungeon.level.drop(new VioletDewdrop(), c).sprite.drop();}
				else if (Random.Int(6)==1){Dungeon.level.drop(new RedDewdrop(), c).sprite.drop();}
				else if (Random.Int(1)==0){Dungeon.level.drop(new YellowDewdrop(), c).sprite.drop();}
			}
		  }	
		}
	}
