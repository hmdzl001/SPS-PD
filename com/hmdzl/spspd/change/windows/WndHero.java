...
public class WndHero extends WndTabbed {
	...
	private class StatsTab extends Group {
		...
		public StatsTab() {
			...
			statSlot(Messages.get(this, "move"), Statistics.moves) ;
			statSlot(Messages.get(this, "time"),  ((int)(Statistics.time/60) + ":" + (int)(Statistics.time%60)));
			
			statSlot(Messages.get(this, "pet"), Dungeon.hero.petCount);
	...
	private class PetTab extends Group {
		...
		public PetTab(final PET heropet) {		
			...
			RedButton btnFree = new RedButton(Messages.get(this, "pet_free")) {
				@Override
				protected void onClick() {
					hide();
					heropet.HP -= heropet.HT/3;
					if(heropet.HP<0){
						heropet.destroy();
						heropet.sprite.killAndErase();
						Dungeon.hero.haspet=false;
						GLog.n(Messages.get(WndHero.class, "pet_getfree"));
						//Dungeon.hero.petCount++;
					} else GLog.n(Messages.get(WndHero.class, "pet_freedmg"));
				}
			};
			btnFree.setRect(btnStay.right() + 1, btnStay.top(),
					btnFree.reqWidth() + 2, btnFree.reqHeight() + 2);

			add(btnFree);

			pos = btnFree.bottom() + GAP;

			statSlot(Messages.get(this, "pet_attack"), heropet.attackSkill(null));
